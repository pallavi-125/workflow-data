...
const WorkflowStep = ({
  ...
  return (
    <Box display="flex" gap={1}>
      ...
      <Accordion
        expanded={isOpen}
        sx={{ width: "100%", "::before": { margin: 0 } }}
      >
        <AccordionSummary>{name_title}</AccordionSummary>
        <AccordionDetails>
          {Object.keys(params_extra)
            .filter((key) => params_extra[key])
            .map((key) => (
              <div key={key}>
                {key}:{" "}
                {typeof params_extra[key] === "object"
                  ? JSON.stringify(params_extra[key])
                  : params_extra[key]}
              </div>
            ))}
        </AccordionDetails>
